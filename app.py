...
def main() -> None:
    ...
    st.set_page_config(page_title="HintCode | Learn by thinking", page_icon="💡", layout="wide")
    ...
    with get_session(settings.database_url) as session:
        ...
        page = st.sidebar.radio("LEARN", list(PAGES.keys()), key="learning_page", on_change=lambda: st.session_state.update(show_problem_library=False))
        st.sidebar.divider()
        st.sidebar.markdown("#### Learning items")
        if st.sidebar.button("Browse problem library", use_container_width=True):
            st.session_state["show_problem_library"] = True
            st.rerun()
        if st.sidebar.button("Continue a problem", use_container_width=True):
